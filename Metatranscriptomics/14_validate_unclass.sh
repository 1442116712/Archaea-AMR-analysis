...
UNCLASS_DIR=${META}/unclassified_arg_contigs_min300
mkdir -p ${UNCLASS_DIR}
...
TOTAL=0
for s in "${SAMPLES[@]}"; do
  bc=${META}/contig_arg_min300/${s}/${s}_arg_bedcov.tsv
  named=${META}/kaiju_out/${s}_contigs_min300/${s}_contigs_kaiju_named.out
  contigs=${META}/assembly/${s}/transcripts_min300.fasta
  
  if [[ ! -s $bc || ! -s $named ]]; then continue; fi
...
echo ""
echo "Total: $TOTAL contigs"
if [[ $TOTAL -gt 0 ]]; then
  samtools faidx ${UNCLASS_DIR}/all_unclass_arg_contigs.fasta
  echo ""
  awk '{
    n++; sum+=$2; if($2>=500)c5++; if($2>=1000)c1k++
    if($2>max)max=$2; if(min==""||$2<min)min=$2
  }
  END {
    printf "Length: n=%d mean=%.0f min=%d max=%d  >=500:%d  >=1000:%d\n",
           n, sum/n, min, max, c5+0, c1k+0
