#!/bin/bash
# Compute TPM_Global and TPM_Archaeal for all 16 samples
# Run on login node — pure awk + bedcov, no SLURM needed
set -euo pipefail
...
for SAMPLE in "${SAMPLES[@]}"; do
  ...
  # Step 1: BED + bedcov (only if any HSPs)
  ...
  # Step 4: per-ARG RPK (global + archaeal)
  ...
  BEGIN { OFS="\t"
    print "Sample","Gene_ID","Length","Reads_All","RPK_All","TPM_Global",
          "Reads_Archaeal","RPK_Archaeal","TPM_Archaeal"
  }
  ...
  ARGIND==2 {
    ...
    printf "%s\t%s\t%d\t%.2f\t%.4f\t%.4f\t%.2f\t%.4f\t%.4f\n",
           sample, arg, len, reads, rpk, tpm_g, ar, arpk, tpm_a
    seen[arg]=1
  }
  ARGIND==3 && !($1 in seen) {
    printf "%s\t%s\t%d\t0\t0.0000\t0.0000\t0\t0.0000\t0.0000\n", sample, $1, $2
  }
  ...
  printf "  >>> %s rows=%d  TPM_Global+=%d  TPM_Archaeal+=%d\n" \
    "$SAMPLE" "$ROWS" "$GP" "$AP"
done
