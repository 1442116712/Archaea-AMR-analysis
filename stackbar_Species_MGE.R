...
data <- read_excel('C:/Users/CFL/Desktop/UK/PhD/AMR/Archaea/ARG_align_prok/Archaea_MGE_nt_prok.xlsx', sheet = "Species")
...
p <- ggplot(long_data_percent, aes(x = factor(1), y = Percentage, fill = Category)) +
  ...
  labs(x = "Species_MGE_ARG", y = "Percentage (%)", 
       title = "Species distribution by Species_MGE_ARG",
       fill = "Species") +
  theme_minimal() +
...
p <- ggplot(long_data_percent, aes(x = factor(1), y = Count, fill = Category)) +
  geom_bar(stat = "identity", position = "stack", width = 0.8) +
  scale_fill_manual(values = c(extended_colors)) +
