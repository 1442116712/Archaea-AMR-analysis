...
data <- read_excel('C:/Users/CFL/Desktop/UK/PhD/AMR/Archaea/ARG_align_prok/Archaea_MGE_nt_prok.xlsx', sheet = "Biosample")
...
p <- ggplot(long_data_percent, aes(x = factor(1), y = Percentage, fill = Category)) +
  ...
  scale_fill_manual(values = c("#F1A7A1", "#F4A582", "#F5D06D", "#A6D854", "#94C5CC", "#00CED1", "#457B9D", "#E6A8D7", "#C3B091")) +
  ...
  facet_wrap(~ ID, ncol = 10, strip.position = "bottom") +  # 按ID分面
  labs(x = "Accession_MGE_ARG", y = "Percentage (%)", 
       title = "Biosample types by Accession_MGE_ARG",
       fill = "Biosample type") +
  theme_minimal() +
...
p <- ggplot(long_data_percent, aes(x = factor(1), y = Count, fill = Category)) +
  geom_bar(stat = "identity", position = "stack", width = 0.8) +
  scale_fill_manual(values = c("#F1A7A1", "#F4A582", "#F5D06D", "#A6D854", "#00CED1", "#EED0C1", "#C3B091", "#808000")) +
  geom_text(aes(label = ifelse(Count > 10, paste0(round(Count), ""), "")),
            position = position_stack(vjust = 0.5), size = 3.5, color = "black", fontface = "plain") +
  facet_wrap(~ ID, ncol = 10, strip.position = "bottom") +  # 按ID分面
  labs(x = "Species_MGE_ARG", y = "Homogenous sequence counts", 
       title = "Biosample types by Species_MGE_ARG",
       fill = "Biosample type") +
  theme_minimal() +
  theme(axis.text.x = element_blank(),  # 隐藏x轴文本，因为每个分面只有一个条形
        axis.ticks.x = element_blank(),  # 隐藏x轴刻度
